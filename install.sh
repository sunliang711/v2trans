...
install(){
    ...
    sed -e "s|START_PRE|${this}/bin/v2trans.sh _start_pre|g" \
        -e "s|V2RAY|${this}/Linux/v2ray|g" \
        -e "s|CONFIG|${this}/etc/config.json|g" \
        -e "s|STOP_POST|${this}/bin/v2trans.sh _stop_post|g" v2trans.service >/tmp/v2trans.service

    _runAsRoot "mv /tmp/v2trans.service /etc/systemd/system"
    _runAsRoot "systemctl daemon-reload"
    _runAsRoot "systemctl enable v2trans.service"
